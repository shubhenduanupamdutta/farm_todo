...
class ListSummary(BaseModel):
    ...
    def from_doc(doc) -> "ListSummary":
        return ListSummary(id=str(doc["_id"]), name=doc["name"], item_count=doc["item_count"])
...
class ToDoDAL:
    def __init__(self, todo_collection: AsyncIOMotorCollection) -> None:
        self._todo_collection = todo_collection

    async def list_todo_lists(self, session=None) -> AsyncGenerator[ListSummary, None]:
    ...
    async def set_checked_state(
        self, doc_id: str | ObjectId, item_id: str, checked_state: bool, session=None
    ) -> ToDoList | None:
        result = await self._todo_collection.find_one_and_update(
    ...
    async def delete_item(
        self, doc_id: str | ObjectId, item_id: str, session=None
    ) -> ToDoList | None:
        result = await self._todo_collection.find_one_and_update(
            {"_id": ObjectId(doc_id)},
            {
